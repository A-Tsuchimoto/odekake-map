...
`rings` は輪の半径(km)で、地域ごとに変えてよい（関東は10/20/30/40、沖縄は10/25/50/75）。
...
- 沖縄は、同じ施設を2つのカテゴリに登録している行がある。台帳のまま両方残してある

同じ施設が2件あるもの（沖縄）。片方に寄せたいなら、記録が付く前に消すこと。
...
元は手作りのExcel台帳2つ。関東は `高田駅_家族お出かけスポット統合` の拡張版
...
Google Maps評価が入っているのは関東の最初の128件だけ（2026-07-26時点の参考値）。
関東の拡張55件と沖縄125件は未確認のため空にしてある（推測では埋めない）。

所要時間のスライダーの上限は、データの最大値から自動で決まる（いまは車155分・
